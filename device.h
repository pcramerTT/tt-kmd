...
struct tenstorrent_device_class {
    ...
    // Thunderbolt-specific callbacks
    int (*tb_setup_tunneling)(struct tenstorrent_device *ttdev);
    void (*tb_cleanup_tunneling)(struct tenstorrent_device *ttdev);
    int (*tb_handle_hotplug)(struct tenstorrent_device *ttdev, bool connected);
    int (*tb_optimize_latency)(struct tenstorrent_device *ttdev);
};
